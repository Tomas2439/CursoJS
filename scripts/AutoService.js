...
function buscador(question,p_buscar){ // se copian las propiedades del objeto a un arreglo
    const valores = Object.keys(cupones);
    if (valores.includes(p_buscar)==true){ // se compara si la palabra esta dentro del arreglo (la misma propiedad del objeto)
        question = true;
    }
    return question;
}
